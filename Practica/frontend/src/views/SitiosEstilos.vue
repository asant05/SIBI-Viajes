<template>
    <v-app>
      ...
      <div style="background-color: #ffffff">
        <div text-align="center" style="margin-left: 50px; margin-right: 50px">
          <br />
          Bienvenido a Viajar es vivir, aqui podemos ver las opciones para el estilo:
          <br />
          <strong>{{this.$store.state.estiloViaje}}</strong>
          <br />
          <br />
        </div>
      </div>
  ...
  export default {
    data: () => ({
      ...
      mounted(){
        //Comprobamos el estilo de viaje que sea
        if(this.$store.state.estiloViaje=='Festividad'){
            //Cogemos la media de fiesta de los sitios de la base de datos
            axios.get('http://localhost:3000/avgFiesta').then(response=>{
              //Introducimos dicho valor en la variable correspondiente para luego hacer la consulta
              this.avgFiesta = response.data[0];
              this.$store.state.avgFiesta = this.avgFiesta
              console.log(" La media de fiesta es "+this.avgFiesta)
                //Para considerar a un sitio de festividad, supongo que tenga que tener una 
                //nota de fiesta mayor a la media
                ...
                axios.get('http://localhost:3000/busquedaFestividad',{
                    params:{
                        avgFiesta: this.$store.state.avgFiesta,
                    }
                }).then(response=>{
                    //AQUI RELLENAMOS EL ARRAY DE SITIOS PARA QUE LOS MUESTRE EN LA TABLA
                    //Cada sitio va a venir acompañado de 10 valores, con lo cual cada 10 i cambiamos de sitio     
        ...
        } else if(this.$store.state.estiloViaje=='Desconexion'){
            //Cogemos la media de fiesta de los sitios de la base de datos
            axios.get('http://localhost:3000/avgNaturaleza').then(response=>{
              //Introducimos dicho valor en la variable correspondiente para luego hacer la consulta
              this.avgNaturaleza = response.data[0];
              this.$store.state.avgNaturaleza = this.avgNaturaleza
              console.log(" La media de naturaleza es "+this.avgNaturaleza)
                //Para considerar a un sitio de festividad, supongo que tenga que tener una 
                //nota de fiesta mayor a la media
                axios.get('http://localhost:3000/busquedaNaturaleza',{
                    params:{
                        avgNaturaleza: this.$store.state.avgNaturaleza,
                    }
                }).then(response=>{
                    //AQUI RELLENAMOS EL ARRAY DE SITIOS PARA QUE LOS MUESTRE EN LA TABLA
                    //Cada sitio va a venir acompañado de 10 valores, con lo cual cada 10 i cambiamos de sitio     
        ...
        } else if(this.$store.state.estiloViaje=='Playa'){
            //Cogemos la solo los sitios con playa, osea de costa, y se ordena mediante la naturaleza
            axios.get('http://localhost:3000/busquedaPlaya').then(response=>{
                //AQUI RELLENAMOS EL ARRAY DE SITIOS PARA QUE LOS MUESTRE EN LA TABLA
                //Cada sitio va a venir acompañado de 10 valores, con lo cual cada 10 i cambiamos de sitio     
                for (var step = 0; step < response.data.length; step+=10) {
                    this.sitios.push({ 
        ...
        }else if(this.$store.state.estiloViaje=='Cultural'){
            //Cogemos la media de fiesta de los sitios de la base de datos
            axios.get('http://localhost:3000/avgMonumento').then(response=>{
              //Introducimos dicho valor en la variable correspondiente para luego hacer la consulta
              this.avgMonumento = response.data[0];
              this.$store.state.avgMonumento = this.avgMonumento
              console.log(" La media de monumento es "+this.avgMonumento)
                //Para considerar a un sitio de cultura, supongo que tenga que tener una 
                //nota de monumentos mayor a la media, y sea turistico
                axios.get('http://localhost:3000/busquedaCultural',{
                    params:{
                        avgMonumento: this.$store.state.avgMonumento,
                    }
                }).then(response=>{
                    //AQUI RELLENAMOS EL ARRAY DE SITIOS PARA QUE LOS MUESTRE EN LA TABLA
                    //Cada sitio va a venir acompañado de 10 valores, con lo cual cada 10 i cambiamos de sitio     
